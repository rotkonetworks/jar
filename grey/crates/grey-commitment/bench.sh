...
echo ""
echo "=== Building ==="
RUSTFLAGS="-C target-cpu=native" cargo build --release -p grey-commitment --example profile 2>&1 | grep -E "Compiling grey-commitment|Finished"
...
RUSTFLAGS="-C target-cpu=native" taskset -c ${CORES} cargo bench -p grey-commitment 2>&1 | grep -E "time:|commitment::"
