...
fn random_poly(size: usize) -> Vec<BinaryElem32> {
    ...
    (0..size)
        .map(|_| BinaryElem32::from(rng.r#gen::<u32>()))
        .collect()
}
...
fn bench_prove(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("sha256", "2^20"), |b| {
        b.iter(|| {
            ...
            let proof = grey_commitment::prove(
                &config,
                black_box(&poly),
                &mut transcript,
            )
...
fn bench_verify(c: &mut Criterion) {
    ...
    let proof = grey_commitment::prove(&prover_config, &poly, &mut pt).unwrap();

    group.bench_function(BenchmarkId::new("sha256", "2^20"), |b| {
        b.iter(|| {
            let mut vt = Sha256Transcript::new(0);
            let valid = grey_commitment::verify(
                &verifier_config,
                black_box(&proof),
                &mut vt,
            )
