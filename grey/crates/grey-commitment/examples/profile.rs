...
//! Run with: RUSTFLAGS="-C target-cpu=native" cargo run --release -p grey-commitment --example profile
...
fn main() {
    ...
    let t0 = Instant::now();
    for _ in 0..5 {
        let tree = grey_commitment::merkle::build_merkle_tree_from_hashes(&hashes);
    ...
    for _ in 0..3 {
        ...
        let _ = grey_commitment::prove(&config, &poly, &mut t).unwrap();
    }
    ...
    for _ in 0..10 {
        ...
        let p = grey_commitment::prove(&config, &poly, &mut t).unwrap();
...
fn rs_encode_columns(
    ...
    rs: &grey_commitment::reed_solomon::ReedSolomon<BinaryElem32>,
) {
    let n = mat[0].len();
    let cols: Vec<Vec<BinaryElem32>> = (0..n)
        .map(|j| {
            let mut col: Vec<BinaryElem32> = mat.iter().map(|row| row[j]).collect();
            grey_commitment::reed_solomon::encode_in_place(rs, &mut col);
            col
        })
        .collect();
