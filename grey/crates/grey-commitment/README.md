...
[![Crates.io](https://img.shields.io/crates/v/grey-commitment.svg)](https://crates.io/crates/grey-commitment)
[![Docs.rs](https://docs.rs/grey-commitment/badge.svg)](https://docs.rs/grey-commitment)
...
let proof = grey_commitment::prove(&prover_cfg, &poly, &mut pt).unwrap();

// Verify
let mut vt = Sha256Transcript::new(0);
let valid = grey_commitment::verify(&verifier_cfg, &proof, &mut vt).unwrap();
assert!(valid);
```
