...
impl<F: BinaryFieldElement> EncodedBlock<F> {
    ...
    /// Convert into a [`Witness`](crate::encode::Witness) for the prover.
    ///
    ...
    pub fn into_witness(self) -> crate::encode::Witness<F> {
        crate::encode::Witness {
            data: self.data,
            rows: self.rows,
            cols: self.cols,
            tree: self.row_tree,
    ...
    /// Borrow as a [`Witness`](crate::encode::Witness) reference without consuming.
    pub fn as_witness(&self) -> crate::encode::Witness<F>
    where
        F: Clone,
    {
        crate::encode::Witness {
