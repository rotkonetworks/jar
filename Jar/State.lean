...
def stateTransitionWithOpaque (s : State) (b : Block)
  ...
  -- Block import validation: epoch marker contents (entropy + validators)
  -- NOTE: Cannot enable — the conformance_forks trace has blocks generated by PolkaJAM
  -- against a specific pre-state. When block 074 is correctly rejected by this check,
  -- subsequent fork-sibling blocks run against a different pre-state where epoch marker
  -- presence/absence doesn't match (shouldEM vs hasEM mismatch), causing 8 cascade failures.
  -- The validation logic itself is correct; it's the sequential fork test runner that
  -- can't reconstruct the correct per-branch state that PolkaJAM used to generate each block.
